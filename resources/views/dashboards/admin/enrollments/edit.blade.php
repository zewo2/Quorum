...
@section('title', 'Edit Enrollment - Quorum')
...
<div class="form-container">
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('dashboard.admin.enrollments.update', $enrollment) }}" class="stacked-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <select id="course_id" name="course_id" required class="@error('course_id') is-invalid @enderror">
                    @foreach($courses as $course)
                        <option value="{{ $course->id }}" {{ (int)old('course_id', $enrollment->course_id) === $course->id ? 'selected' : '' }}>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" min="0" max="20" id="final_grade" name="final_grade" value="{{ old('final_grade', $enrollment->final_grade) }}" class="@error('final_grade') is-invalid @enderror" placeholder="0 - 20">
                    @error('final_grade')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-group">
                ...
                <textarea id="notes" name="notes" rows="3" class="@error('notes') is-invalid @enderror" placeholder="Add optional notes">{{ old('notes', $enrollment->notes) }}</textarea>
                @error('notes')
                    <span class="error-message">{{ $message }}</span>
                @enderror
            </div>
